...
const sizeTable: Record<SizeKey, CapsuleSize> = {
  ...
  [pageStateMachine.createKey(PageState.Music, "collapsed")]: [340, 50] as CapsuleSize,
  [pageStateMachine.createKey(PageState.Music, "expanded")]: MUSIC_EXPANDED_SIZE,
  [pageStateMachine.createKey(PageState.Music, "seeking")]: MUSIC_EXPANDED_SIZE,
...
function resolveTargetSize(): CapsuleSize {
  ...
  const state = pageStateMachine.substates[page]?.state;
  if (state) {
...
export function initrAF() {
  listen<boolean>("set-hover", (event) => {
    pageStateMachine.isHover = event.payload;
    event.payload ? capsule.classList.add("hover") : capsule.classList.remove("hover");
  });
