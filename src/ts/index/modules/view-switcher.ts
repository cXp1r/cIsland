...
// ---------------------------------------------------------------------------
// 闂傚倷绀侀幉锟犳偡椤栫偛鍨傞柟鎯版閺嬩線鏌曢崼婵囧闁哥姴妫濋弻娑㈠焺閸愮偓鐣风紓浣稿€搁悧鎾诲蓟濞戙垹绠抽柟鍨暞閻ｄ粙姊洪棃娑欘棞闁哥喐鎸冲顐㈩吋婢跺﹪鏁滈梺璋庡懐澧ch 婵犵數鍋為崹鍫曞箰閸濄儳鐭撻柡澶嬪焾閸ゆ洘銇勯幒宥堝厡闁崇粯妫冮獮鏍垝閻熸澘鈷夐梺绋垮濞茬喖寮婚敐鍜佺叆閹艰揪绱曟禒鈺呮⒑閹肩偛濡肩紒缁橈耿閻涱噣骞嬮敃鈧～鍛存煟濡搫鏆辨い蹇ｅ灦閺岀喖鎮℃惔锝嗘喖濠电偠灏欓崰鏍х暦?dots闂?
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 闂備礁婀遍崢褔鎮洪妸銉冩椽鎮㈤悡搴ｏ紵闁诲酣娼ч幉锟犲窗閸℃稒鐓曢柍鈺佸枤濞堟洜绱掗崒姘卞ⅵ闁哄本鐩獮鎺楀箣閻愬樊妲遍柣搴ゎ潐濞叉﹢鎮￠垾宕囨殾?UI
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 婵犳鍠楀畷鍧楀川椤撳缍侀弻锝夊冀閻㈤潧鍩屽銈庡幑閸斿矂锝炲┑鍡欐殾闁搞儯鍔庨埀顒夊弮濮婃椽宕ㄦ繝鍌滅懆濠电偠顕滅粻鎴︽偩閻戣姤鍤戞い鎺嶇閺嬪倿姊虹化鏇炲⒉妞ゃ劌妫涚划?
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// DOM 闂傚倷绀佸﹢閬嶅煕閸儱绀堟繝闈涚墛瀹曡尙鈧箍鍎遍ˇ浼村疾椤掍焦鍙忔慨妤€妫楁晶鐗堢箾閸垹鍔嬬紒缁樼洴瀹曪絾寰勭€ｅ灚鍋ョ紓鍌欑閻牓宕滃☉銏犵疅闁圭虎鍠栭獮銏′繆閵堝嫯鍏屾い鈺婂幘缁辨挻鎷呴崜鍙壭銈嗘肠閸パ呯厬?#current-view闂傚倷鐒︾€笛呯矙閹达附鍤愭い鏍仜閻鏌ｉ幇闈涘濠殿垰鐡ㄧ换娑㈠幢濡ゅ啰顔夌紓浣插亾闁逞屽墴濮婃椽宕崟顒夋！闂侀潧娲ㄩ崑鐔煎礆?
// ---------------------------------------------------------------------------
...
function mountView(mode: ViewMode) {
  ...
  if (el) {
    ...
    // 婵?display:none 闂備浇顕ф绋匡耿闁秴绠犻柟鐐灱閺嬪秹鏌熼悜妯烩拻闁活厽鎹囬幃褰掑炊閵娿儳绁峰┑顕嗙到椤︾敻寮诲☉姗嗘僵妞ゆ帒瀚烽埀顒侇殘閳ь剝顫夐幃鍌涚鐠鸿櫣鏆︽慨妯挎硾闁卞洭鏌￠崶鈺佷沪妞ゅ繒鍠撶槐鎾存媴閸濆嫅锝嗐亜閵娿儲顥㈤柟顔ㄥ洤纭€闁绘垵妫楀▓?
    el.style.display = "flex";
  }
}

// ---------------------------------------------------------------------------
// 闂傚倷绀侀幖顐﹀疮閻楀牊鍙忛悗娑櫳戦崣蹇涙煃閸濆嫭鍣洪柛瀣ㄥ姂閹綊宕堕妸銉хシ闂佽楠搁妶鎼佸蓟閻旂儤瀚氶柍鈺佸暟缁愭瑧绱撴担浠嬪摵闁荤啿鏅涢悾宄邦潩椤戣姤鏂€闂佺硶妾ч弲婊呯礊韫囨稒鈷?/ 闂佽瀛╅鏍窗閺嶎厼绠规い鎰剁畱閺勩儲淇婇妶鍛殲鐎规洖顦甸幃妤呭捶椤撶倫锝囩磼娓氬洤娅嶉柡?
// ---------------------------------------------------------------------------

export function showOnlyView(mode: ViewMode) {
  // 闂傚倷绀侀幉锟犳偡閿曞倹鍋嬫俊銈呭暟閻捇鏌ｉ幋锝嗩棄缂佺姰鍎甸弻宥堫檨闁告挾鍠庨锝夊垂椤愩垻绐為柟鍏兼儗閸犳绱炴繝鍥ㄢ拺闁硅偐鍋涢崝姘辨喐閹殿喖浠х紒顕呭弮瀹曟帒顫濋敐鍡欎簴闂佽鍑界紞鍡涘礈濮橆兘鏋旂€光偓閸曨剛鍘介梺闈涱焾閸庨亶顢旈埡浣叉斀妞ゆ棁顫夊▍濠囨煙?
...
// ---------------------------------------------------------------------------
// 闂備焦鐪归崺鍕垂閻ｅ瞼涓嶉柟瀵稿仦閸欏繘鏌嶉崫鍕櫤闁稿鍔戦幃褰掑炊閵娿儳绁烽梺璇查閵堟悂寮?
// ---------------------------------------------------------------------------
...
function animateViewSwitch(from: ViewMode, to: ViewMode) {
  ...
  // 闂傚倷鑳堕…鍫㈡崲閹扮増鍋嬪┑鐘插暕缁诲棝鏌熼梻瀵割槮缂侇偄绉堕幉鎼佸箣閿旇偐绋忛柣搴秵閸犳牠鎮欐繝鍥ㄧ厽闁逛即娼ф晶鑼磼閹邦収娈滈柡?
  if (fromEl && fromEl.parentElement === currentViewContainer) {
    ...
    outAnim.onfinish = () => {
      ...
      // 闂傚倷绀佸﹢閬嶅煕閸儱绀堟繛鍡楃贩濞差亜鍐€妞ゆ挾鍋炴潏鍫濃攽閿涘嫬浠滃褑妫勯埢?
  ...
  // 闂傚倷绀侀幖顐﹀磹鐟欏嫬鍨斿ù鐘差儜缂嶆牠鎮楅敐搴℃灈闁绘劕锕弻锝夊箛闂堟稑顫╅梺浼欏瘜閸ㄥ爼寮诲☉婊呯杸閻庯綆浜滄慨搴☆渻閵堝棙鈷愰柛鏃€鐟ラ悾鐑藉箮閽樺）鈺呮煏婢跺牆濡奸柣婵愬灠閳规垿鎮滈崶鈺佺煗闂侀€炲苯澧柛鐔风仢椤?
...
export function updateCapsuleSize() {
  pageStateMachine.applyPageClasses(pageStateMachine.getCurrentPage(), capsule.classList);
}

// ---------------------------------------------------------------------------
// 缂傚倸鍊搁崐鐑芥嚄閸洖绐楃€广儱娲ㄩ崡姘舵煙缂併垹鏋涚紒鈧崒鐐寸厪濠㈣泛鐗嗛崝姘辩磽瀹ュ棙宕岄柡灞诲妼閳藉鈻庨幒鎴濠电姭鎷冮崨顓夈儳绱掗瑙勬珚妞ゃ垺绋戦湁闁靛牆瀚粣鏃傗偓瑙勬礈椤牐鐏冮梺鍛婃处閸樹粙顢欏澶嬧拺?
// ---------------------------------------------------------------------------

export function setView(mode: ViewMode, animated = true) {
  const previous = currentView;//闂傚倸顭崑鍕归崒鐐茬；闁糕剝绋戦惌?
  if (isManualPageState(previous)) {
    pageStateMachine.savePageClasses(previous, capsule.classList);
  }
  setCurrentView(mode);
  // 婵犵數濮烽。浠嬪焵椤掆偓閸熷潡鍩€椤掆偓缂嶅﹪骞冨Ο璇茬窞閻庯綆鍓﹀ù?agent 闂備浇顕х换鎺楀磻閻愯娲冀椤愶綆娼熼梺纭呮彧缁犳垹绮婚幋婢濆綊鏁愰崶鍓佸姼闂佽楠搁妶鎼佸箖绾拋妲婚柟鐓庣摠缁骸危閹邦兘鏀介悗锝庝簽閸濇姊虹拠鈥崇仯濠⒀勵殔閻ｇ兘鎼归顐ｎ啍闂佺粯鍔樺▔娑㈡偂閹邦優褰掓偑閳ь剟宕板Δ鈧銉╁礋椤愵偄鎮戦梺鎼炲劗閺呮盯鎮欐繝鍥ㄢ拺闁告稑锕﹂幊鏇犵磼闊厾鐭欏┑锛勬暬瀹曞爼鍩￠埀顒勬倿?
  ...
  // 婵犵數濮烽。浠嬪焵椤掆偓閸熷潡鍩€椤掆偓缂嶅﹪骞冨Ο璇茬窞閻庯綆鍓﹀ù?lyric 闂備浇顕х换鎺楀磻閻愯娲冀椤愶綆娼熼梺纭呮彧缁犳垹绮婚幋婢濆綊鏁愰崶鍓佸姼闂佽楠搁妶鎼佸箖绾拋妲婚柟鐓庣摠缁骸危閹邦兘鏀介悗锝庝簽閸濇姊虹拠鈥崇仯濠⒀勵殔閻?
  ...
  // 婵犵數濮烽。浠嬪焵椤掆偓閸熷潡鍩€椤掆偓缂嶅﹪骞冨Ο璇茬窞閻庯綆鍓﹀ù?sadb 闂傚倷绀侀幉锛勬暜閹烘嚚娲煛閸涱喗鍟掗柣搴秵閸犳寮查鍌楀亾閸忓浜鹃梺閫炲苯澧伴柛鎺撳浮楠炴﹢骞囨担瑙勩€冮梻渚€娼х换鍫ュ磹閺嵮€鏋旈柡鍐ㄧ墛閻撴洜鈧箍鍎遍幊鎰不娴煎瓨鐓犻柤濮愬€曢弸搴ㄦ煏閸℃ê娴€规洏鍔戦、娑樷槈濡嘲浜鹃柣鎰劋閻撴瑧鐥悧鍩亝绂嶆ィ鍐┾拺?sadb 闂?
  if (previous === "sadb" && mode !== "sadb") {
    ...
    if (capsule.classList.contains("sadb-idle")) {
      ...
      // 闂傚倷绀侀幉锟犳嚌閹灐褰掓倻缁涘鏅滃銈嗗笒鐎氼剛绮堟径鎰厪濠电倯鍐仾婵絽閰ｅ娲箹閻愭彃濮㈤梺绋款儍閸婃宕氶幒妤婃晬婵犲﹤瀚娑㈡⒑闂堟稓澧曢柟鍐茬箰鍗遍柍褜鍓熼弻锝嗘償閿濆棙姣勫銈庡幖閻楁捇骞?snap 闂傚倷鐒﹂幃鍫曞磿閹惰棄纾婚柕鍫濐槸杩濇繝鐢靛Т濞诧箓鎮?
      window.setTimeout(() => {
  ...
  if (isManualPageState(mode)) {
    pageStateMachine.applyPageClasses(mode, capsule.classList);
    pageStateMachine.syncPageState(mode, capsule.classList);
  }
...
// ---------------------------------------------------------------------------
// 闂傚倷绀佸﹢閬嶆惞鎼淬劌鍌ㄥù鐘差儏濡ê銆掑锝呬壕閻庤娲栭悥濂稿箖濞嗘搩鏁嗛柛灞炬皑娴煎洭姊绘担鍛婂暈闁煎綊绠栧鐢割敆閸曞灚鏅?
// ---------------------------------------------------------------------------
...
export function updatePlayIcon() {
  ...
  // 闂傚倸鍊搁崐鎼佹偋閸曨垰鍨傞柛婵嗗閻斿棙鎱ㄥ璇蹭壕濡ょ姷鍋為敃銏ゃ€佸鈧幃銏ゅ川婵犲啰鐣梻鍌欑劍閹爼宕曢幎钘夌；闁瑰墽绮崑澶愭煥濠靛棭妲搁悷娆欑畵閹﹢鎮欐担鍐╊€楅梺?
