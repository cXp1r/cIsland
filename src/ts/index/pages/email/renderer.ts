...
function debugEmail(message: string, data?: unknown): void {
  if (data === undefined) {
...
function sanitizeMailHtml(html: string): string {
  ...
  doc.querySelectorAll("script, iframe, object, embed, link[rel='import']")
...
async function loadMailboxList(): Promise<void> {
  ...
  const metas = await invoke<EmailMeta[]>("fetch_email_metas_by_uids", {
    uids: allUids.slice(0, 20),
  });
...
async function ensureFirstSelected(): Promise<void> {
  debugEmail("ensureFirstSelected:start", {
    activeUid,
    allUids: allUids.length,
    cachedMetas: cachedMetas.length,
  });
  if (activeUid) return;
...
async function selectMail(meta: EmailMeta): Promise<void> {
...
export async function showEmbeddedEmailView(): Promise<void> {
...
export function initEmailRenderers(): void {
  debugEmail("initEmailView");
  emailRefreshBtn.addEventListener("click", () => {
    void refreshMailbox();
  });
  emailClearCacheBtn.addEventListener("click", () => {
    void clearCache();
  });
  emailContent.addEventListener("click", (event) => {
    ...
    if (!link) return;
    const targetUrl = new URL(link.getAttribute("href") || "", window.location.href).toString();
...
export const emailList: Record<string, PageRenderSpec> = {
  collapsed: {
    classList: null,
    size: [140, 50],
  },
  expanded: {
    ...
    size: () => {
      ...
      return [
        parseInt(style.getPropertyValue("--email-view-w"), 10),
        parseInt(style.getPropertyValue("--email-view-h"), 10),
      ];
    },
  },
};
